...
import { ICountry, IRegion } from "@/lib/types"
...
import { updateRegion } from "@/lib/data/region"
...
const EditRegionForm = ({ region, countries }: Props) => {
    ...
    const onSubmit = async (values: regionSchemaValues) => {
        ...
        const [, err] = await updateRegion(region._id, values)
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                                            ...
                                            {filteredCountries.map(country => (
                                                <div key={country._id} className="flex gap-2 items-center text-sm">
                                                    <Checkbox
                                                        ...
                                                        defaultChecked={region.countries.includes(country._id)}
                                                    />
                                                    <Label
                                                        htmlFor={country._id}
                ...
                />
                <Button disabled={loading} type="submit" className="lg:col-span-2">Update region</Button>
            </form>
        </Form>
    )
}
