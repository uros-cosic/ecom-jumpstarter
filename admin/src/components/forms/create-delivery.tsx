...
import { createShippingMethod } from "@/lib/data/delivery"
...
const CreateDeliveryForm = ({ regions }: Props) => {
    ...
    const onSubmit = async (values: shippingMethodSchemaValues) => {
        ...
        const [, err] = await createShippingMethod(values)
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                ...
                />
                <Button disabled={loading} type="submit" className="lg:col-span-2">Create shipping method</Button>
            </form>
        </Form>
    )
}
